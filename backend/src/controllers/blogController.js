...
export const getBlogs = asyncHandler(async (req, res) => {
  ...
  res.json(ApiResponse.success('Blogs retrieved successfully', {
    blogs,
    pagination: {
      page,
      limit,
...
export const getBlog = asyncHandler(async (req, res) => {
  ...
  res.json(ApiResponse.success('Blog retrieved successfully', blogWithLikeStatus));
});
...
export const updateBlog = asyncHandler(async (req, res) => {
  ...
  if (blog.author.toString() !== req.user._id.toString()) {
    return res.status(403).json(ApiResponse.forbidden('Not authorized to update this blog'));
  }
...
export const deleteBlog = asyncHandler(async (req, res) => {
  ...
  // Check if user owns the blog
  if (blog.author.toString() !== req.user._id.toString()) {
    return res.status(403).json(ApiResponse.forbidden('Not authorized to delete this blog'));
  }
...
export const getMyBlogs = asyncHandler(async (req, res) => {
  ...
  res.json(ApiResponse.success('My blogs retrieved successfully', {
    blogs,
    pagination: {
      page,
      limit,
...
export const togglePublish = asyncHandler(async (req, res) => {
  ...
  res.json(ApiResponse.success(`Blog ${blog.status === 'published' ? 'published' : 'unpublished'} successfully`, blog));
});
...
export const toggleLike = asyncHandler(async (req, res) => {
  ...
  if (existingLike) {
    ...
    res.json(ApiResponse.success('Blog unliked successfully', { liked: false }));
  } else {
